...
rates = [5, 10, 15, 20, 25, 50, 75, 100, 125, 150, 175, 200]
...
# Create a set to store unique flyids
activated_ids = set()

# Loop over the input intensity
for vp in vp_list:
    for i in rates:
        # Read the CSV and add flyids to the set
        flyids = pd.read_csv('../results/cellTypes/CSVs/'+str(vp.lower())+'_'+str(i)+'Hz_rates.csv')['flyid'].values
        activated_ids.update(flyids)
        
# Convert set to DataFrame and save to CSV
activated_ids_df = pd.DataFrame(list(activated_ids), columns=['root_id'])
activated_ids_df.to_csv('activated_ids.csv', index=False)

# Read activated IDs from CSV and convert to set
#activated_ids = set(pd.read_csv('activated_ids.csv')['root_id'].values)
...
def process_row(idx, row, meshes):
    ...
    try:
        ...
        for glom_name, mesh in meshes.items():
            ...
            if trimesh_mesh.contains([point])[0] and row['pre_pt_root_id'] in activated_ids:
                result[glom_name] = 1
    except IndexError:
...
# Function to process a single row
def process_input_row(idx, row, meshes):
    result = {glom_name: 0 for glom_name in meshes.keys()}  # Initialize all glomerulus columns to 0
    try:
        point = row[['post_pt_position_x', 'post_pt_position_y', 'post_pt_position_z']].values
        for glom_name, mesh in meshes.items():
            trimesh_mesh = mesh.trimesh
            if trimesh_mesh.contains([point])[0] and row['post_pt_root_id'] in activated_ids:
                result[glom_name] = 1
    except IndexError:
        print(f"Index error occurred at idx {idx}")
    return idx, result
...
def process_input_synapse_df(syn_df, meshes):

    # Parallel processing with progress bar
    with parallel_backend("loky", inner_max_num_threads=1):  # Use loky backend for better thread management
        results = Parallel(n_jobs=-1)(
            delayed(process_input_row)(idx, row, meshes)
            for idx, row in tqdm(syn_df.iterrows(), total=len(syn_df), desc="Processing rows")
        )
...
alpn_syn_df.to_csv('./output/activation_analysis/alpn_glomeruli_output.csv', index=False)
...
alpn_summed_df.to_csv('./output/activation_analysis/alpn_glomeruli_output_sum.csv', index=False)
...
# Save the processed dataframes to CSV
other_syn_df.to_csv('./output/activation_analysis/other2N_glomeruli_output.csv', index=False)

# Group by root_id and sum glomeruli columns for all three dataframes
other_summed_df = other_syn_df.groupby('pre_pt_root_id').sum(numeric_only=True).reset_index()

# Save the summed dataframes
other_summed_df.to_csv('./output/activation_analysis/other2N_glomeruli_output_sum.csv', index=False)
...
# Save the processed dataframes to CSV
olf3N_syn_df.to_csv('./output/activation_analysis/olfactory3N_glomeruli_output.csv', index=False)
...
# Save the summed dataframes
olf3N_summed_df.to_csv('./output/activation_analysis/olfactory3N_glomeruli_output_sum.csv', index=False)



###  Olfactory 3N Neurons Input  ###

# Process olfactory synapses
olf3N_syn_df = process_input_synapse_df(olf3N_syn_df, meshes)

# Save the processed dataframes to CSV
olf3N_syn_df.to_csv('./output/activation_analysis/olfactory3N_glomeruli_input.csv', index=False)
...
# Save the summed dataframes
olf3N_summed_df.to_csv('./output/activation_analysis/olfactory3N_glomeruli_input_sum.csv', index=False)
